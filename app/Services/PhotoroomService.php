...
class PhotoroomService
{
    ...
    public const FRAMING_FIELDS = [
         ...
         * moved off Luggage kept sizing by "the body" — measuring a dress from
         * halfway down its skirt, where the flare first reaches half the
         * widest row.
         */
        'body_fill'          => null,
    ];
     ...
     * and each of their notes says so — jeans, jacket and t-shirt are the
     * three menswear entries measured off a real sample rather than assumed,
     * and all three agreed with the house rule.
     */
    public const FRAMING_PRESETS = [
         ...
         * entries to correct. Jacket, t-shirt and jeans below are measured off
         * real samples rather than assumed, and all three agreed with the
         * house rule exactly.
         */
        'men' => [
            'label'         => 'Men',
            'subcategories' => [
                ...
                'jacket'   => ['label' => 'Jackets',   'note' => 'Measured off a finished catalogue frame (STR202TOP04893, a Stefano Ricci hooded jacket) on a 2000 square: the jacket sits 10.00% down, ends 9.90% up from the bottom, and fills 80.10% of the height. The same house rule already confirmed on jeans, now confirmed on a second menswear category.', 'edits' => ['width' => 2000, 'height' => 2000, 'padding' => 0.10]],
                't-shirt'  => ['label' => 'T-shirts',  'note' => 'Measured off a finished catalogue frame (CIN202TOP04225) on a 2048 square: the t-shirt sits 10.01% down, ends 9.86% up from the bottom, and fills 80.13% of the height — the same house rule again, on a third menswear category.', 'edits' => ['width' => 2000, 'height' => 2000, 'padding' => 0.10]],
    ...
    private function applySegmentation(array &$fields, array $edits): void
    {
         ...
         * be the salient object — and on a garment displayed on a dress form,
         * that judgement takes in the form. The negative prompt then loses the
         * argument and the stand survives the cutout, which is the one thing
         * naming the product was meant to prevent. Measured on a poncho: the
         * background came away cleanly and the mannequin was still standing in
         * it, wearing the garment.
         *
         ...
         *
         * This was briefly made conditional — aggressive for a word somebody
         * typed, gentle for a word this app guessed — because a guessed word
         * that matches nothing selects nothing, and the whole frame comes back.
         * That was solving the right problem in the wrong place. The gentle mode
         * does not rescue a bad guess; it just fails differently, keeping the
         * stand instead of the studio. What rescues a bad guess is noticing that
         * nothing was cut out and asking again without the word, which
         * EditPhotoItemJob now does — and that makes the confident mode safe to
         * use on a guess, because there is something behind it when it misses.
         */
